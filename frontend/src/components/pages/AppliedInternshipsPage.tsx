...
const SortableInternshipCard = ({ item }: { item: Application }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} {...attributes} className="glass-card p-4 touch-none">
            ...
            <div className="mt-3 pt-3 border-t border-white/10 flex justify-between items-center">
                ...
                <p className="text-xs text-neutral-400">Applied: {new Date(item.appliedDate).toLocaleDateString()}</p>
            </div>
        </div>
    );
};
...
const AppliedInternshipsPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchApplied = async () => {
            setIsLoading(true);
            try {
                const data = await api.getAppliedInternships(user.email);
    ...
    // Normalise to lowercase to match API response values
    const columns: Record<string, Application[]> = {
        'applied': applications.filter(app => app.status === 'applied'),
        'review': applications.filter(app => app.status === 'review'),
                        ...
                        <div className="grid md:grid-cols-3 gap-6 mt-8">
                            {Object.entries(columns).map(([key, items]) => (
                                <div key={key} className="glass-card p-4">
                                    <h3 className="font-bold text-white px-2 pb-3">{columnLabels[key]} ({items.length})</h3>
