...
import { UploadCloud, Search, Home, MapPin, Building, CheckCircle, ChevronDown, PlusCircle, Check, Sparkles } from 'lucide-react';
...
const InternshipFinderPage = () => {
    ...
    const [resumeParsed, setResumeParsed] = useState<{ skills?: string[]; ai_powered?: boolean } | null>(null);
    ...
    const handleResumeUpload = async (file: File) => {
        ...
        try {
            ...
            const { data } = await apiClient.post('/resume/parse-and-fill', formData, {
    ...
    return (
                            ...
                            <div className="flex items-center">
                                {steps.map((s, index) => (
                                    <React.Fragment key={index}>
                                        ...
                                        {index < steps.length - 1 && <div className={`flex-1 h-1 mx-4 transition-all ${index + 1 < step ? 'bg-primary' : 'bg-neutral-700'}`} />}
                                    </React.Fragment>
                                ))}
                            </div>
                        </header>
                                ...
                                {step === 1 && (
                                    <div className="p-8 space-y-6">
                                        ...
                                        <InputField
                                            ...
                                            onChange={(e) => setPreferences(p => ({ ...p, locations: e.target.value }))}
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between bg-neutral-800/50 p-3 rounded-lg border-2 border-neutral-700">
                                                    ...
                                                    <button
                                                        onClick={() => setPreferences(p => ({ ...p, isRural: !p.isRural }))}
                                                ...
                                                <div className="relative">
                                                    <select
                                                        ...
                                                        onChange={(e) => setPreferences(p => ({ ...p, socialCategory: e.target.value }))}
                                    ...
                                    <div className="p-8 space-y-6">
                                        ...
                                        <div>
                                            ...
                                            >
                                                ...
                                                <span className={`font-semibold ${resume ? 'text-green-400' : 'text-neutral-400'}`}>
                                             ...
                                             {isUploading && (
                                                <p className="text-xs text-primary mt-2 flex items-center gap-1">
                                                   <Sparkles size={12} className="animate-pulse" /> AI parsing your resume...
                                                </p>
                                             )}
                                             {resumeParsed?.ai_powered && resumeParsed.skills && (
                                                <p className="text-xs text-green-400 mt-2 flex items-center gap-1">
                                                   <Check size={12} /> AI extracted {resumeParsed.skills.length} skills and auto-filled your profile!
                                                </p>
                                             )}
     {error && <p className="text-red-400 text-sm mt-2">{error}</p>}
                                        ...
                                        <div className="flex justify-between pt-4">
                                            ...
                                            <AnimatedButton onClick={handleSubmit} disabled={isSubmitting} icon={isSubmitting ? undefined : Search}>
                                                {isSubmitting ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : 'Find Matches'}
                                            </AnimatedButton>
                                        </div>
                                    </div>
                                )}
                                ...
                                {step === 3 && (
                                    <div className="p-8">
                                                    ...
                                                    >
                                                        <div className="flex-grow">
                                                            ...
                                                            <div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-sm text-neutral-400 mt-1">
                                                                ...
                                                                {match.source && (
                                                                    <span className="bg-primary/20 text-primary text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-bold border border-primary/30 ml-2">{match.source}</span>
                                                                )}
                                                            </div>
                                                        </div>
                                                        <div className="text-left sm:text-right w-full sm:w-auto mt-2 sm:mt-0 shrink-0 flex sm:flex-col items-center sm:items-end gap-2">
                                                            {match.score !== undefined && (
                                                                <p className="font-bold text-lg text-green-400">{Math.round(match.score * 100)}% Match</p>
                                                            )}
                                                            <AnimatedButton
                                        ...
                                        <div className="flex justify-center mt-8">
                                            <button
                                                onClick={() => { setStep(1); setMatches(null); setResume(null); }}
